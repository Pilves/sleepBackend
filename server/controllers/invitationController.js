...
module.exports = (firestoreUtils) => {
  ...
  const getAllInvitations = async (req, res) => {
    try {
      ...
      // Use the dedicated firestoreUtils function for invitations
      const invitations = await firestoreUtils.getAllInvitations(
  ...
  const validateInvitationCode = async (req, res) => {
    try {
      ...
      logger.info(`validateInvitationCode started for code: ${code}`, { requestId });
      ...
      if (!invitation) {
        ...
        const response = { valid: false, error: 'Invalid invitation code', requestId };
      ...
      if (invitation.status !== 'PENDING') {
        ...
        const response = { valid: false, error: 'This invitation has already been used or revoked', requestId };
        logger.info(`Sending response:`, response);
      ...
      if (new Date() > invitation.expiresAt) {
        ...
        const response = { valid: false, error: 'This invitation has expired', requestId };
        logger.info(`Sending response:`, response);
        return res.status(400).json(response);
      }

      logger.info(`Invitation code ${code} is valid for ${invitation.email}`, { requestId });
      const successResponse = { valid: true, email: invitation.email, expiresAt: invitation.expiresAt };
      ...
      const errorResponse = { valid: false, error: 'Failed to validate invitation code', message: error.message, requestId: req.id };
